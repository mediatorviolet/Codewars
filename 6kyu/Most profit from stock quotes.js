 ...
 *
 * Stock quotes are stored in an array in order of date. The stock profit is the difference in prices in buying and selling stock. Each day, you can either buy
 * one unit of stock, sell any number of stock units you have already bought, or do nothing. Therefore, the most profit is the maximum difference of all pairs in
...
const getMostProfitFromStockQuotes = (quotes) => {
    ...
    while (quotes.length !== 0) {
        ...
        let n = quotes.lastIndexOf(max);
        maxProfit = quotes
            .slice(0, n)
            .reduce((pv, cv) => (pv += max - cv), maxProfit);
